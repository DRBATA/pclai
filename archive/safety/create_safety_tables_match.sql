...
-- Adjusted to match existing table structure
...
-- Safety Tags Table for categorization - using tag_category and tag_value structure
...
-- Create indexes on safety_tags for quick lookups
CREATE INDEX IF NOT EXISTS idx_safety_tags_category ON safety_tags(tag_category);
CREATE INDEX IF NOT EXISTS idx_safety_tags_value ON safety_tags(tag_value);
CREATE INDEX IF NOT EXISTS idx_safety_tags_combined ON safety_tags(tag_category, tag_value);
